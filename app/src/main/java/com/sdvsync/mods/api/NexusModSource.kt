...
class NexusModSource(
    ...
    override suspend fun getDownloadUrl(modId: String, fileId: String): String = withContext(Dispatchers.IO) {
        val apiKey = getApiKey() ?: throw IllegalStateException("No API key configured")

        val request = Request.Builder()
            .url("$BASE_URL/games/$GAME_DOMAIN/mods/$modId/files/$fileId/download_link.json")
            .header("apikey", apiKey)
            .get()
            .build()
